...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    async def lifespan(app: FastAPI) -> AsyncIterator[None]:
        ...
        # Состояние лабораторных стендов: какой вариант поднят и какие
        # подсказки уже открыты. В памяти, а не в базе, намеренно — стенд
        # живёт ровно столько же, сколько процесс, и переживать перезапуск
        # ему нечем. Итог попытки уезжает в attempts и сохраняется.
