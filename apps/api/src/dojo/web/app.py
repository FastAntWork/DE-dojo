...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    async def lifespan(app: FastAPI) -> AsyncIterator[None]:
        ...
        # Состояние лабораторных стендов: какой вариант поднят. В памяти, а не
        # в базе, намеренно — стенд живёт ровно столько же, сколько процесс, и
        # переживать перезапуск ему нечем. Итог попытки уезжает в attempts.
        app.state.lab_stands = {}
        # Открытые подсказки живут отдельно от стенда: перезапуск стенда не
        # должен снимать штраф за уже прочитанную подсказку.
        app.state.lab_hints = {}
